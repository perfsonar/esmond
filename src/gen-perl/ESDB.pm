...
$self->{resolution} = undef;
  if (UNIVERSAL::isa($vals,'HASH')) {
    ...
    if (defined $vals->{iface_name}) {
      $self->{iface_name} = $vals->{iface_name};
    }
    if (defined $vals->{oidset}) {
      $self->{oidset} = $vals->{oidset};
    }
...
sub read {
  ...
  while (1) 
  {
    ...
    SWITCH: for($fid)
    {
      ...
      last; };
      /^2$/ && do{      if ($ftype == TType::STRING) {
        $xfer += $input->readString(\$self->{iface_name});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
      /^3$/ && do{      if ($ftype == TType::STRING) {
        $xfer += $input->readString(\$self->{oidset});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
      /^4$/ && do{      if ($ftype == TType::STRING) {
        $xfer += $input->readString(\$self->{oid});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
...
sub write {
  ...
  if (defined $self->{iface_name}) {
    $xfer += $output->writeFieldBegin('iface_name', TType::STRING, 2);
    $xfer += $output->writeString($self->{iface_name});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{oidset}) {
    $xfer += $output->writeFieldBegin('oidset', TType::STRING, 3);
    $xfer += $output->writeString($self->{oidset});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{oid}) {
    $xfer += $output->writeFieldBegin('oid', TType::STRING, 4);
    $xfer += $output->writeString($self->{oid});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{begin_time}) {
    $xfer += $output->writeFieldBegin('begin_time', TType::STRING, 5);
    $xfer += $output->writeString($self->{begin_time});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{end_time}) {
    $xfer += $output->writeFieldBegin('end_time', TType::STRING, 6);
    $xfer += $output->writeString($self->{end_time});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{flags}) {
    $xfer += $output->writeFieldBegin('flags', TType::STRING, 7);
    $xfer += $output->writeString($self->{flags});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{cf}) {
    $xfer += $output->writeFieldBegin('cf', TType::STRING, 8);
    $xfer += $output->writeString($self->{cf});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{resolution}) {
    $xfer += $output->writeFieldBegin('resolution', TType::STRING, 9);
...
sub select{
  ...
  my $device = ($request->{'device'}) ? $request->{'device'} : undef;
  my $iface_name = ($request->{'iface_name'}) ? $request->{'iface_name'} : undef;
  my $oidset = ($request->{'oidset'}) ? $request->{'oidset'} : undef;
  my $oid = ($request->{'oid'}) ? $request->{'oid'} : undef;
  my $begin_time = ($request->{'begin_time'}) ? $request->{'begin_time'} : undef;
  my $end_time = ($request->{'end_time'}) ? $request->{'end_time'} : undef;
  ...
  return $self->{impl}->select($device, $iface_name, $oidset, $oid, $begin_time, $end_time, $flags, $cf, $resolution);
}
...
sub process_select{
    ...
    my $result = new ESxSNMP::ESDB_select_result();
    eval {
      $result->{success} = $self->{handler}->select($args->device, $args->iface_name, $args->oidset, $args->oid, $args->begin_time, $args->end_time, $args->flags, $args->cf, $args->resolution);
